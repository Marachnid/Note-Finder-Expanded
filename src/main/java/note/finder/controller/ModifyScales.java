...
//        processRequest(request, nameValidation, scaleDao, newScale);
    ...
    private void processRequest(HttpServletRequest request, String nameValidation,
            NoteFinderDao<MusicalScale> scaleDao, MusicalScale newScale) {
    ...
    private MusicalScale returnNewScaleObject(HttpServletRequest request) {
        ...
        //this is a temporary solution, need to add form values
        int category = Integer.parseInt(request.getParameter("category"));
        NoteFinderDao<MusicalCategory> categoryDao = new NoteFinderDao<>(MusicalCategory.class);


        MusicalCategory categoryObject = categoryDao.getById(category);

        return new MusicalScale(name, root, second, third, categoryObject);
    }
}
